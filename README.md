...
Tabulation System for Miss Duran (Nabua, Camarines Sur)
...
3. Clone or download this repository to your XAMPP **htdocs** folder.
   The final path should be `path_to/xampp/htdocs/msduran-nabua`.
...
5. Inside [phpMyAdmin](http://localhost/phpmyadmin),
   create a MySQL database named `msduran-nabua` and import [msduran-nabua.sql](msduran-nabua.sql) into it.

6. Open the terminal and navigate to the project directory **msduran-nabua**.
...
9. Open your web browser and access <http://localhost:5180/msduran-nabua/> to view the application.
...
2. Inside [phpMyAdmin](http://localhost/phpmyadmin),
   create a MySQL database named `test-msduran-nabua` and import [msduran-nabua.sql](msduran-nabua.sql) into it.
...
2. Access the application by visiting `http://[host_name]/msduran-nabua`,
   where `host_name` is the **IP address** or **host name** of the server in the network.
   For example:
     - <http://localhost/msduran-nabua>
     - <http://192.168.1.99/msduran-nabua>


#### Admin Dashboard
Access the **admin dashboard** at <http://localhost/msduran-nabua/app>.
